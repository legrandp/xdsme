...
__date__ = "31-08-2017"
__copyright__ = "Copyright (c) 2017 Pierre Legrand"
...
class Interpreter:
    ...
    'BeamX':(['PILT_SPATIAL_BEAM_POSITION'], lambda x: float(x.split()[0])),
    'BeamY':(['PILT_SPATIAL_BEAM_POSITION'], lambda x: float(x.split()[1])),
    'Distance':(['PILT_GONIO_VALUES'], lambda x: float(x.split()[-1])),
    'Wavelength':(['SCAN_WAVELENGTH'], float),
    'PixelX':(['PILT_SPATIAL_DISTORTION_INFO'], lambda x: float(x.split()[-2])),
    'PixelY':(['PILT_SPATIAL_DISTORTION_INFO'], lambda x: float(x.split()[-1])),
    'Width':(['SIZE1'], int),
    'Height':(['SIZE2'], int),
    'Message':(['COMMENT'], str),
    ...
    'EdgeResolution':(['PILT_DETECTOR_SIZE','PILT_GONIO_VALUES','SCAN_WAVELENGTH'],
                                                            getEdgeResolution),
    'HeaderSize':(['HEADER_BYTES'], int),
    'EndianType':(['BYTE_ORDER'], endian),
    # Added keys from Graeme's convention.
    'TwoTheta':(['PILT_GONIO_VALUES'], lambda x: float(x.split()[1])),   # _FIXME_ Not really here now...
    'SerialNumber':(['PILT_SERIAL_NUMBER'], str),
    'HeaderSize':(['HEADER_BYTES'], int),
    'EndianType':(['BYTE_ORDER'], endian),
